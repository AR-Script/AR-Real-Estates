...
export const google = async (req, res, next) => {
    try {
        ...
        if (user) {
            const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET); //sign token
            ...
            const generatedPassword = Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8); //generate 16digit password for Google sign in
